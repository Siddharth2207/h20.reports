...
const OrdersTable = () => {
  ...
  const filteredDailyData = dailyData.filter((order) => {
    const ownerMatches =
      !ownerFilter || order.owner.toLowerCase().includes(ownerFilter.toLowerCase());
    const statusMatches =
      orderStatus === '' ||
      (orderStatus === 'active' ? order.active === true : order.active === false);
    ...
    <div className="w-full overflow-x-auto rounded-lg bg-white shadow-lg">
      <div className="flex rounded-t-lg border-b border-gray-300 bg-gray-100">
        {['24h', 'weekly'].map((tab) => (
          ...
          >
            {tab === '24h' ? '24h Activity' : 'Weekly Activity'}
          </button>
        ))}
      </div>

      {/* Filters Section */}
      <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 p-4 shadow-md">
        {/* Owner Filter Input */}
        <div className="flex items-center">
          <label className="mr-3 text-sm font-semibold text-gray-700">Filter by Owner:</label>
          <input
            type="text"
            className="w-80 rounded-lg border border-gray-300 p-2 text-sm text-gray-900 placeholder-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        {/* Order Status Filter */}
        <div className="flex items-center">
          <label className="mr-3 text-sm font-semibold text-gray-700">Status:</label>
          <select
            className="rounded-lg border border-gray-300 p-2 text-sm text-gray-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            value={orderStatus}
                            ...
                            <span>
                              ...
                              {formatBalance(
                                order.inputBalances.reduce((sum, input) => {
                                  return (
                                    sum +
                                    parseFloat(input.inputTokenBalance) *
                                      parseFloat(
                                        order.tokenPriceMap[input.inputTokenAddress.toLowerCase()],
                                      )
                                  );
                                }, 0),
                              )}
                            ...
                            <span>
                              ...
                              {formatBalance(
                                order.outputBalances.reduce((sum, output) => {
                                  return (
                                    sum +
                                    parseFloat(output.outputTokenBalance) *
                                      parseFloat(
                                        order.tokenPriceMap[
                                          output.outputTokenAddress.toLowerCase()
                                        ],
                                      )
                                  );
                                }, 0),
                        ...
                        {order.volume24H.length > 0 ? (
                          order.volume24H.map((input, index) => (
                            ...
                            >
                              ...
                              <span className="text-gray-800">
                                {formatBalance(input.totalVolume)}
                              </span>
                            </div>
                          ))
                        ) : (
