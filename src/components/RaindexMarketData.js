...
const prepareTradeAndVolumeStats = async (tradesAccordingToTimeStamp, orderTrades) => {
  ...
  const totalExternalVolume = externalTrades.reduce(
  ...
  const allDates = Array.from(
    new Set([...Object.keys(raindexGrouped), ...Object.keys(externalGrouped)]),
  ).sort();
  ...
  const tradeStats = [{ name: 'Raindex' }, { name: 'External' }];

  const volumeStats = [{ name: 'Raindex' }, { name: 'External' }];
...
const RaindexMarketData = () => {
  ...
  const fetchAndSetData = async (token, fromTimestamp, toTimestamp) => {
    try {
      ...
      const oldestTimestamp = Math.min(...allOrders.map((i) => Number(i.timestampAdded)));
      ...
      }
      {
        const {
          ...
          allTradesArray.filter((i) => {
            return i.timestamp >= fromTimestamp && i.timestamp <= toTimestamp;
          }),
          raindexOrderWithTrades.filter((i) => {
            return i.timestamp >= fromTimestamp && i.timestamp <= toTimestamp;
          }),
        );
  ...
  const renderBarChart = (
    ...
    const cardSpanClass =
      cardSpan === 3 ? 'col-span-3' : cardSpan === 2 ? 'col-span-2' : 'col-span-1';

    // Calculate Y-axis domain
    const maxVal = Math.max(...dataSets.flatMap((data) => data.map((item) => item.total || 0)));
    const yAxisMax = maxVal + maxVal * 0.1;

    return (
      <div className={`flex flex-col rounded-lg bg-white p-5 shadow-lg ${cardSpanClass}`}>
        {/* Chart Title */}
        <h3 className="mb-2 text-center text-lg font-semibold text-gray-800">{title}</h3>

        {/* Subtitle (optional) */}
        {subtitles && <p className="mb-4 text-center text-sm text-gray-600">{subtitles}</p>}
          ...
          {dataSets.map((data, index) => (
            <div key={index} className="flex flex-col">
              <ResponsiveContainer width="100%" height={cardHeight}>
                <BarChart data={data} margin={{ top: 10, right: 10, bottom: 20, left: 25 }}>
                  ...
                  {colorKeys.map((key, keyIndex) => (
                    <Bar key={key} dataKey={key} stackId="a" fill={bluePalette[keyIndex]} />
                  ))}
              ...
              <div className="mt-4">
                {colorKeys.map((key, keyIndex) => {
                  ...
                  return (
                    <div key={keyIndex} className="mb-2 flex items-center">
                      <div className="w-20 text-sm font-semibold text-gray-700">{key}</div>
                      <div className="relative mx-2 h-2 flex-1 rounded-full bg-gray-200">
                        <div
  ...
  const renderInsights = (
    ...
    const pieDataTrades = [
      {
        name: 'Raindex',
        value: totalRaindexTrades,
        percentage: ((totalRaindexTrades / totalTrades) * 100).toFixed(1),
      },
      {
        name: 'External',
        value: totalExternalTrades,
        percentage: ((totalExternalTrades / totalTrades) * 100).toFixed(1),
      },
    ];
    ...
    const pieDataVolume = [
      {
        name: 'Raindex',
        value: totalRaindexVolume,
        percentage: ((totalRaindexVolume / totalVolume) * 100).toFixed(1),
      },
      {
        name: 'External',
        value: totalExternalVolume,
        percentage: ((totalExternalVolume / totalVolume) * 100).toFixed(1),
      },
    ];
              ...
              {pieDataTrades.map((stat, index) => (
                <div key={index}>
                  <div className="mb-1 flex justify-between">
                    ...
                    <span>
                      {formatValue(stat.value)} - {stat.percentage}%
                    </span>
                  </div>
